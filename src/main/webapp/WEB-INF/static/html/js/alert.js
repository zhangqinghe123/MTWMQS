...
(function(window,$){
     ...
     * @param text 需要显示的文本
     */
    fakemr.showSuccess = function (text,url) {
        _showMainDiv();
        $('#_fakeMr_Message_Text').append(getMessage_TextDiv(text), getMessage_suButton());
        successCallback(url);
    };
    /**
     * 通知
    ...
    function getMessage_TextDiv(_str) {
        ...
        var cssStr = { 'font-size': '14px', 'padding': '15px 8px','color':'#4a4a4a' };
        return _fakeMr_Text.css(cssStr);
    }
    /*成功跳转回调*/
    function successCallback(url){
        $("#_fakeMr_success").attr("href",url);
    }
    /*关闭Alert窗口事件*/
    function BindCloseMessageEven() {
        $("#_fakeMr_close").tap(function () {
            _closeWinDiv();
        });
    }
    ...
    function BindCloseConfirmEven(leftCallback, rightCallback) {
        $("#_fakeMr_Confirm_leftButton").tap(function () {
            _closeWinDiv();
            (leftCallback && typeof(leftCallback) === "function") && leftCallback();
        });
        $("#_fakeMr_Confirm_rightButton").tap(function () {
            _closeWinDiv();
    ...
    /*成功回调按钮*/
    function getMessage_suButton() {
        var _fakeMr_suButton = $('<a id="_fakeMr_success"></a>');
        var cssStr = {
            'font-size': '14px',
            'color':'#1ee89f'
        };
        ...
        return _fakeMr_suButton.css(cssStr).append(pDom);
    }
...
/*Message.showConfirm("确定***吗？","是","不是",function(){
 //第一个回调
 },function(){
 //第二个回调
 });*/