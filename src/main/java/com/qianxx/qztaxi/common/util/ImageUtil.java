...
public class ImageUtil {
	...
	public static String createWaterMarkWithImage(String srcImgPath, String outputImage, String waterMarkImgPath,
			String zone, float transparency, float imageQuality, int watermarkMinWidth, int watermarkMinHeight) throws IOException {
		imageQuality /= 100;
		...
		graphics.drawImage(watermark_image, zoneArray[0], zoneArray[1], waterMarkImgWidth, waterMarkImgHeight, null,
				null);
		graphics.dispose();
	...
	public static String createWaterMarkWithCharacter(String srcImagePath, String outputImagePath,
			int watermarkminwidth, String watermarktext_text, String watermarktext_fontpath, int watermarktext_size,
			...
			String watermarktext_translatey, String watermarktext_skewx, String watermarktext_skewy, float imageQuality,
			String watermarkstatus, float watermarktrans) throws IOException {
		watermarktrans /= 100;
		imageQuality /= 100;
		...
		if (watermarktext_shadowcolor != null && !watermarktext_shadowcolor.trim().equals("")) {
			...
			graphics2D.drawString(watermarktext_text, zoneArray[0] + watermarktext_shadowx,
					zoneArray[1] + watermarktext_shadowy);
		}
		try {
	...
	private static int[] getWriteZoneForImage(String zone, int width, int height, int waterMarkImgWidth,
			int waterMarkImgHeight) {
		int distance = 5;
	...
	public static String createZoomImage(String sourceFilePath, String targetFileName, int width, int height)
			throws IOException {
		...
		if (oldHeight > height || oldWidth > width) {
			...
			width = src_image.getWidth(null);
			height = src_image.getHeight(null);
			BufferedImage bufferedImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
			Graphics graphics = bufferedImage.createGraphics();
			graphics.drawImage(src_image, 0, 0, width, height, null);
			graphics.dispose();
	...
	public static String createZoomImage(File targetFile, File thumbFile, int width, int height) throws IOException {
		Image src_image = ImageIO.read(targetFile);
		double oldHeight = src_image.getHeight(null);
		double oldWidth = src_image.getWidth(null);
		if (oldHeight > height || oldWidth > width) {
			double ratio = 0;
	...
	private static boolean isCattoon(String srcImgPath) throws IOException {
		...
		while ((currentByte = srcFis.read()) != -1) {
			if (log == 0 && currentByte == 0x21 || log == 1 && currentByte == 0xff || log == 2 && currentByte == 0x0b
					|| log == 3 && currentByte == 'N' || log == 4 && currentByte == 'E'
					|| log == 5 && currentByte == 'T' || log == 6 && currentByte == 'S'
					|| log == 7 && currentByte == 'C' || log == 8 && currentByte == 'A'
					|| log == 9 && currentByte == 'P' || log == 10 && currentByte == 'E'
					|| log == 11 && currentByte == '2' || log == 12 && currentByte == '.'
